...
# --- ROTACIÓN DE CLAVES DE GEMINI (PRINCIPAL Y SECUNDARIA) ---
LISTA_API_KEYS = [
    "AQ.Ab8RN6KsUkcoD82gau9CS49g9ZTIFnO9d3lkje0JXdDiULJpXg",  # Clave Principal
    "AQ.Ab8RN6LexBaJvbFnxfSSjUax0BMDxExEoqlMwrhkhODaY4JF-g"   # Clave Secundaria
]

def consultar_gemini(prompt):
    """
    Consulta a Gemini probando las claves en orden.
    Si la clave principal se agota, pasa de forma automática a la secundaria.
    """
    modelos = ['gemini-2.5-flash', 'gemini-1.5-flash', 'models/gemini-2.5-flash']
    
    for i, key in enumerate(LISTA_API_KEYS):
        if not key:
            continue
        try:
            ...
            print(f"⚠️ Clave API #{i+1} agotada. Probando la siguiente clave...")
            continue
        except Exception as e:
            print(f"⚠️ Error en Clave API #{i+1}: {e}")
            continue

    return "❌ Todas las claves de API de Gemini han alcanzado su límite diario por hoy."
...
class Yarvis:
    ...
    def aprender_autonomamente(self):
        ...
        conocimiento = consultar_gemini(prompt)
        if "❌" not in conocimiento:
            clave_memoria = f"dato sobre {tema}"
            self.datos["memoria"][clave_memoria] = conocimiento.strip()
            self.guardar_datos()
    ...
    def responder(self, entrada):
        ...
        # Consulta a la IA con respaldo de claves
        return consultar_gemini(texto)
...
def bucle_auto_aprendizaje():
    while True:
        time.sleep(900)  # Revisa cada 15 minutos para preservar las cuotas
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("¡Yarvis activo en la nube! Sistema de doble clave API configurado.")
...
if __name__ == "__main__":
    ...
    print("=== YARVIS EN LA NUBE CON ROTACIÓN DE CLAVES ===")
