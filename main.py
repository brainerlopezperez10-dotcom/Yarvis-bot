...
# --- RUTA PRINCIPAL CON SISTEMA DE DIAGNÓSTICO ---
@app.route('/')
def home():
    try:
        ...
        # Muestra en la pantalla qué carpetas y archivos existen en el servidor
        archivos_actuales = os.listdir('.')
        archivos_templates = os.listdir('./templates') if os.path.exists('./templates') else '❌ LA CARPETA TEMPLATES NO EXISTE'
        
        return f"""
        <div style="font-family: sans-serif; padding: 20px; background: #111; color: #fff;">
            <h1 style="color: #ef4444;">⚠️ Error al cargar la plantilla:</h1>
            <p><b>Mensaje de error:</b> {str(e)}</p>
            <hr style="border-color: #333;">
            <h3>📁 Estado de los archivos en el servidor:</h3>
            <p><b>Archivos en la raíz del proyecto:</b> {archivos_actuales}</p>
            <p><b>Archivos dentro de la carpeta /templates:</b> {archivos_templates}</p>
        </div>
        """
