...
@app.route('/')
def home():
    tot_ing = sum(i["monto"] for i in yarvis.datos["ingresos"])
    tot_gas = sum(g["monto"] for g in yarvis.datos["gastos"])
    balance = tot_ing - tot_gas
    
    return render_template('index.html', 
                           memoria=yarvis.datos["memoria"], 
                           notas=yarvis.datos["notas"],
                           balance=balance)
...
if __name__ == "__main__":
    ...
    print(f"=== YARVIS WEB ONLINE EN PORT {port} ===")
